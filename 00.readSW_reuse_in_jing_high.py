...
def read_sw_csv(dirpath_to_csv,match_len_dic,high_high_sid):
	...
	for i,file in enumerate(list_of_sw_csv):
		...
		with open (file,"r",encoding='utf-8') as csvfile:
			...
			for work2, sid1, sid2, score, align1, align2, text1, text2, begin1,end1,begin2,end2 in rows:
				# 若high_high_index已到最後一筆 = high_high_sid已全部跑完就只判斷是否為INGORE
				# 若(sid1,sid2)句對為high_high類別則不紀錄
				if high_high_index!=len(high_high_sid) and (sid1,sid2) == high_high_sid[high_high_index]:
					# print("{},{}為high,high".format(sid1,sid2))
					high_high_index += 1
					continue
				# 若align 內容有 數字, 英文字，全部都剔除
				elif re.search(INGORE_PATTERN, align1) or re.search(INGORE_PATTERN, align2):
					continue
					
				
				# if high_high_index == len(high_high_sid):
					# print("sid1:{},sid2:{}".format(sid1,sid2))
				...
				#如果換經 就紀錄work1 work2的重用字數與句數 並清空紀錄句號 進行下一經的統計
					...
					# print("work1:{},work2:{},work1_word_len:{},work2_word_len:{}".format(work1,now_work2,work1_word_len,work2_word_len))
				...
				# print("-"*40)
				# print(result_list)
	# if high_high_index != len(high_high_sid)+1:
		# for i in range(high_high_index:len(high_high_sid)+1):
			# print(high_high_sid[i])
		# exit()
	return result_list
...
#讀取分類為high_high的sid句對
with open("out/high_high_sid.pickle","rb") as f:
	high_high_sid = pickle.load(f)

# print("high_high_sid長度:{}".format(len(high_high_sid)))
# print("high_high_sid最後一個值:{}".format(high_high_sid[len(high_high_sid)-1]))

# for h in high_high_sid:
	# print(h)

#讀取 sw result csv, 並計算數量
sw_dirpath = os.path.join(PWD,SOURCE_BASE,SW_DATA_DIR)
result_list = read_sw_csv(sw_dirpath,match_len_dic,high_high_sid)
