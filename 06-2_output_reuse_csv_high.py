...
if __name__ == '__main__':
	...
	t0 = datetime.now()
	print("讀取reuse_dict_high.joblib")
	...
	fr.close()
	'''
	reuse_dict = joblib.load('out/reuse_dict_high.joblib')
	# reuse_dict = joblib.load('reuse_dict_test.joblib')
	t1 = datetime.now()
	print("讀取reuse_dict_high完成:{} sec".format((t1-t0).seconds))
	# print(reuse_dict)
	# input(">>>")
	
	#讀取經文重用pair及其重用句號的dic
	print("讀取merge_pair_sen_dic_high.joblib")
	merge_pair_sen_dict = joblib.load('out/merge_pair_sen_dict_high.joblib')
	# merge_pair_sen_dict = joblib.load('merge_pair_sen_dict_test.joblib')
	t2 = datetime.now()
	print("讀取merge_pair_sen_dict_high完成:{} sec".format((t2-t1).seconds))
	...
	while True:
		...
		with open("out/reuse_high/{}_{}_reuse_high_{}.csv".format(inputA,inputB,dic_len), 'w+', encoding='utf-8', newline='') as f:
		...
		for fas_key,fas_value in fas_sen.items():
			...
			for sen_pair in fas_value:
				...
				with open("out/reuse_high/{}_{}_reuse_high_{}.csv".format(inputA,inputB,dic_len), 'a+', encoding='utf-8', newline='') as f:
					writer = csv.writer(f, delimiter=',')
					if jingA != inputA:
